...
# Nodified script from Elliot Jordan's guide found here: hhttps://github.com/homebysix/jss-filevault-reissue
...
promptMessage="Howdy folks! Your Mac's FileVault encryption key needs to be escrowed by Level IT.
...
# Set logo path
levelLogo="/usr/local/jamf/lvl-icon.png"

# Convert POSIX path of logo icon to Mac path for AppleScript.
logoPosix="$(/usr/bin/osascript -e 'return POSIX file "'"$levelLogo"'" as text')"
...
if [[ "$bailOut" == "true" ]]; then
    echo "[ERROR]: $reason"
    launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$failMessage: $reason" -button1 'OK' -defaultButton 1 -startlaunchd &>/dev/null &
    exit 1
fi

# Display a branded prompt explaining the password prompt.
echo "Alerting user $currentUser about incoming password prompt..."
/bin/launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$promptMessage" -button1 "Next" -defaultButton 1 -startlaunchd &>/dev/null
...
until /usr/bin/dscl /Search -authonly "$currentUser" "$userPass" &>/dev/null; do
    ...
    if (( try >= 5 )); then
        ...
        /bin/launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$forgotPwMessage" -button1 'OK' -defaultButton 1 -startlaunchd &>/dev/null &
        exit 1
    fi
done
...
if [[ $fdesetupResult -ne 0 ]]; then
    ...
    /bin/launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$failMessage: fdesetup exited with code $fdesetupResult. Output: $fdesetupOutput" -button1 'OK' -defaultButton 1 -startlaunchd &>/dev/null &
    ...
    echo "Displaying \"failure\" message..."
    /bin/launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$failMessage: New key generated, but escrow did not occur." -button1 'OK' -defaultButton 1 -startlaunchd &>/dev/null &
else
    [[ -n "$fdesetupOutput" ]] && echo "$fdesetupOutput"
    echo "Displaying \"success\" message..."
    /bin/launchctl "$lMethod" "$lId" sudo -u "$currentUser" "$jamfHelper" -windowType "utility" -icon "$levelLogo" -title "$promptTitle" -description "$successMessage" -button1 'OK' -defaultButton 1 -startlaunchd &>/dev/null &
fi
